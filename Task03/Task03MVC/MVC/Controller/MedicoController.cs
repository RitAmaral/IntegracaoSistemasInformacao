...
namespace Task03MVC.MVC.Controller
{
    internal class MedicoController //Controller/Controlador
    {
        ...
        public void MostrarDetalhes(int details) //quando chamarmos este método no programa, vamos ver os detalhes que aparecem na view do médico no método escreverdetalhesmedico.
        {
            ...
            _view.EscreverDetalhesMedico(medico.Funcao, medico.ID, medico.Nome, medico.Genero, medico.Localidade, medico.DataNasc, medico.CartaConducao, medico.Vencimento, medico.CC, medico.NIF, medico.Telemovel, medico.Email, medico.NomeUtil, medico.GetPrivatePassword());
        }
        /*
        ...
        */
        public bool Login(int details) //alterado em relação ao projeto, para incluir aqui os console.readline.
        {
            ...
            if (medico.GetPrivatePassword() == _password)
            {
        ...
        public string GetPassword(int details) //método para aceder à password
        {
            MedicoModel medico = _model.ObterDetalhes(details);
            _view.MensagemGetPassword(medico.Nome, medico.GetPrivatePassword());
            return medico.GetPrivatePassword(); 
        }
        public void Logout(int details) //método para terminar sessão
            {
                MedicoModel medico = _model.ObterDetalhes(details);
        ...
        public bool SetPassword(int details) //método para alterar a password, não está a funcionar
        {
            ...
            if(resp == 'S' || resp == 's')
            {
                ...
                medico.SetPrivatePassword(novaPassword);
                _view.MensagemSetPassword(medico.GetPrivatePassword());
                return true;
            }
            else 
            { 
