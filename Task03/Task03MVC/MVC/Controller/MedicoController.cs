...
namespace Task03MVC.MVC.Controller
{
    internal class MedicoController //Controller/Controlador
    {
        ...
        */
        ...
        public string GetPassword(int details) 
        {
            MedicoModel medico = _model.ObterDetalhes(details);
            _view.MensagemGetPassword(medico.Nome, medico.Password);
            return medico.Password; 
        }
        ...
        public void SetPassword(int details)
        {
            ...
            if(resp == 'S' || resp == 's')
            {
                ...
                medico.Password = Console.ReadLine();
                _view.MensagemSetPassword(medico.Password);
            }
            else 
            { 
